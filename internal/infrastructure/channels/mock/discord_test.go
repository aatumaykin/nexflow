...
// TestDiscordConnector_StartStop tests Start and Stop methods
func TestDiscordConnector_StartStop(t *testing.T) {
	ctx := context.Background()

	tests := []struct {
		name    string
		setup   func(*DiscordConnector)
		wantErr bool
		errMsg  string
	}{
		{
			name:    "successful start",
			setup:   func(c *DiscordConnector) {},
			wantErr: false,
		},
		{
...
// TestDiscordConnector_SendResponse tests sending responses
func TestDiscordConnector_SendResponse(t *testing.T) {
	ctx := context.Background()
...
// TestDiscordConnector_UserOperations tests GetUser and CreateUser
func TestDiscordConnector_UserOperations(t *testing.T) {
	ctx := context.Background()
	userID := "user-1"
	...
	tests := []struct {
		...
		errMsg        string
		checkUser     bool
		action        func(*DiscordConnector, context.Context) (*entity.User, error)
	}{
		{
			...
			checkUser:     true,
			action: func(c *DiscordConnector, ctx context.Context) (*entity.User, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				channelType := valueobject.Channel("discord")
...
// TestDiscordConnector_IncomingChannel tests incoming messages channel
func TestDiscordConnector_IncomingChannel(t *testing.T) {
	conn := NewDiscordConnector()
...
// TestDiscordConnector_IsRunning tests IsRunning method
func TestDiscordConnector_IsRunning(t *testing.T) {
	conn := NewDiscordConnector()
...
// TestDiscordConnector_SendTestMessage tests sending test messages
func TestDiscordConnector_SendTestMessage(t *testing.T) {
	ctx := context.Background()
	conn := NewDiscordConnector()
	_ = conn.Start(ctx)
	...
	tests := []struct {
		...
		},
		{
			...
			userID:    "user-3",
			channelID: "channel-3",
			content:   "This is a very long message that should still be handled correctly for Discord.",
		},
	}
...
// TestDiscordConnector_GetResponses tests GetResponses method
func TestDiscordConnector_GetResponses(t *testing.T) {
	ctx := context.Background()
	conn := NewDiscordConnector()
	_ = conn.Start(ctx)
...
// TestDiscordConnector_MultipleSenders tests concurrent sends
func TestDiscordConnector_MultipleSenders(t *testing.T) {
	ctx := context.Background()
	conn := NewDiscordConnector()
	_ = conn.Start(ctx)
...
// TestDiscordConnector_NilPointer tests nil pointer handling
func TestDiscordConnector_NilPointer(t *testing.T) {
	var conn *DiscordConnector
