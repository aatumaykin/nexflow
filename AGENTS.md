...
3. **Если обнаружен подозрительный файл:**
   - НЕ добавляйте его в commit
   - Уведомите пользователя с указанием файла
   - Предложите добавить в `.gitignore`
   - Пример: `"⚠️ Файл server не должен быть закоммичен. Добавьте его в .gitignore или удалите перед коммитом."`
...
5. **Правило проверки перед commit:**
   - Запустите `git status --short`
   - Проверьте каждый файл на соответствие списку запрещенных
   - Если файл содержит локальные данные — уведомите пользователя
   - Проверьте `.gitignore` на наличие нужных паттернов

**Примеры запрещенных файлов:**
- `coverage.out` — coverage файлы
- `server` — бинарник
- `data.db` — база данных
- `/Users/username/Projects/ProjectName/config.yml` — файлы с абсолютными путями
- `.beads/export-state/` — локальное состояние beads
